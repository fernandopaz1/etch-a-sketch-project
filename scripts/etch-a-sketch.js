...
function addGridFile(cantColumnas) {
    ...
    document.getElementById("gridContainer").innerHTML += '<br class="break">';
}
...
function isAvalidSize(size) {
    ...
    if (size <= 300 || size > 960) return false;
    return true;
}
...
document.getElementById("btnChangeSize").addEventListener("click", () => {
    ...
    while (!isAvalidSize(size)) {
        alert(
            "Invalid value of size. Please input a number between 300 and 960"
        );
